...
class ItemForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'photo': forms.FileInput(attrs={'class': 'hidden', 'id': 'photo-upload'}),
    ...
    def clean(self):
        ...
        # Validate that found items have verification question
        if item_type == 'found':
            if not cleaned_data.get('verification_question'):
                self.add_error('verification_question', 'Please add a verification question for found items')
        
        # Validate that lost items have date_lost
        if item_type == 'lost':
            if not cleaned_data.get('date_lost'):
